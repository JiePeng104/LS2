...
from utils.aggregate_block.dataset_and_transform_generate import dataset_and_transform_generate, get_transform
...
import utils.bd_dataset_adap_blended as bd_adap
import random
...
def add_args(parser):
    ...
    parser.add_argument('--cover_rate', type=float, default=0.01, help='cover image rate')
...
def main():
    ...
    # set cover index
    train_cover = np.zeros_like(train_pidx)
    clean_id = []
    for i in range(len(train_pidx)):
        if train_pidx[i] < 1:
            clean_id.append(i)
    random.shuffle(clean_id)
    c_num = len(train_pidx) * args.cover_rate
    for i in range(int(c_num)):
        train_cover[clean_id[i]] = 1

    # torch.save(train_cover,
    #            args.save_path + '/train_cover_list.pickle',
    #            )

    ### generate train dataset for backdoor attack


    adv_train_ds = bd_adap.prepro_cls_DatasetBD(
    ...
    sam_train = args.sam_train
    if sam_train:
        args.client_optimizer = 'sam'
    ...
    if 'load_path' not in args.__dict__:
        ...
        adv_train_ds = prepro_cls_DatasetBD(
            ...
            poison_idx=train_pidx,  # one-hot to determine which image may take bd_transform
            bd_image_pre_transform=None,
            bd_label_pre_transform=None,
            ori_image_transform_in_loading=tf_compose,
            ori_label_transform_in_loading=None,
            add_details_in_preprocess=True,
            end_pre_process=lambda img_list: [Image.fromarray(np.uint8(img)) for img in img_list]
        )
