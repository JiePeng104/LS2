...
def shrink(args, model, device, criterion, mask_opt, data_loader):
    model.eval()
    nb_samples = 0
...
def Regularization(model):
    L1 = torch.tensor(0).to(args.device).float()
    L2 = torch.tensor(0).to(args.device).float()
...
def mask_train_awm(args, model, device, criterion, mask_opt, data_loader):
    model.eval()
    total_correct = 0
    total_loss = 0.0
    nb_samples = 0
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        # logging.info(f"loss_noise | {loss_rob.item()} ,  | loss_clean | , {loss_nat.item()},  | L1 | , {L1.item()}")
...
def awm(args, result, config):
    ...
    args.ratio = args.samples / data_all_length
    ...
    logging.info(f"Validation set length: {len(data_set_clean)}")
    ...
    args.inner_iters = int(args.samples / args.batch_size) * args.inner
    ...
    cl_test_loss, cl_test_acc = test(args=args, model=net, criterion=criterion, data_loader=clean_test_loader)
    po_test_loss, po_test_acc = test(args=args, model=net, criterion=criterion, data_loader=poison_test_loader)
    logging.info(f'Poisoned Model : PoisonLoss-{po_test_loss}, PoisonACC-{po_test_acc}')
    logging.info(f'Poisoned Model : CleanLoss-{cl_test_loss}, CleanACC-{cl_test_acc}')

    for i in range(args.shrink_steps):
        start = time.time()
        lr = mask_optimizer.param_groups[0]['lr']
        train_loss, train_acc = shrink(args, model=net, device=args.device, criterion=criterion, data_loader=clean_val_loader,
    ...
    for i in range(args.outer):
        ...
        train_loss, train_acc = mask_train_awm(args, model=net, device=args.device,
                                               criterion=criterion, data_loader=clean_val_loader, mask_opt=mask_optimizer)
        cl_test_loss, cl_test_acc = test(args=args, model=net, criterion=criterion, data_loader=clean_test_loader)
        po_test_loss, po_test_acc = test(args=args, model=net, criterion=criterion, data_loader=poison_test_loader)
        end = time.time()

        logging.info('Iter \t\t PoisonLoss \t PoisonACC \t CleanLoss \t CleanACC')
        logging.info('EPOCHS {} \t {:.4f} \t {:.4f} \t {:.4f} \t {:.4f}'.format(
...
if __name__ == '__main__':
    ### 1. basic setting: args
    args = get_args()
    os.chdir('/mnt/BackdoorBench-main')
    print(os.getcwd())
    ...
    if args.checkpoint_save is None:
        args.checkpoint_save = save_path + '/record/defence/awm/'
        if not (os.path.exists(os.getcwd() + args.checkpoint_save)):
            os.makedirs(os.getcwd() + args.checkpoint_save)
    if args.log is None:
        args.log = save_path + '/saved/awm/'
    ...
    ### 3. anp defense:
    result_defense = awm(args, result, config)
    ...
    #
    # if not (os.path.exists(os.getcwd() + f'{save_path}/awm/')):
    #     os.makedirs(os.getcwd() + f'{save_path}/awm/')
    # torch.save(
    #     {
