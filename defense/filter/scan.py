...
def get_features(data_loader, model):
    ...
    model.eval()
    with torch.no_grad():
        for i, (ins_data, ins_target, *additional_info) in enumerate(tqdm(data_loader)):
            ins_data = ins_data.cuda()
            outputs = model(ins_data)
            penultimate_outputs = model.get_penultimate()
            output_np = penultimate_outputs.data.cpu().numpy()
            target_np = ins_target.data.cpu().numpy()
            feats.append(output_np)
            class_indices.append(ins_target)

        feats = np.concatenate(feats, axis=0).tolist()
        class_indices = np.concatenate(class_indices, axis=0).tolist()
