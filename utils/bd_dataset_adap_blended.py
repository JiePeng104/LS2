...
class prepro_cls_DatasetBD(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self,
                 ...
                 end_pre_process: Optional[Callable] = lambda img_list: [Image.fromarray(img.astype(np.uint8)) for img
                                                                         in img_list],

                 ori_image_transform_in_loading: Optional[Callable] = None,
                 ori_label_transform_in_loading: Optional[Callable] = None,
                 cover_idx: Sequence = None,  # one-hot to determine which image may take bd_transform
                 ):
        logging.info('dataset must have NO transform in BOTH image and label !')
        self.dynamic_ls = False
        self.smooth_rates = None
        ...
        if cover_idx is not None:
            assert len(cover_idx) == len(poison_idx)
            for i in range(len(poison_idx)):
                if poison_idx[i] > 0:
                    assert not poison_idx[i] == cover_idx[i]
    ...
    def prepro_backdoor(self):
        ...
        for original_idx, content in enumerate(tqdm(self.dataset, desc=f'pre-process bd dataset')):
            ...
            a = self.cover_idx[original_idx]
            b = self.poison_idx[original_idx]

            img, label = content

            if self.bd_image_pre_transform is not None and self.cover_idx is not None \
                    and self.cover_idx[original_idx] == 1:
    ...
    def set_dynamic_smooth_rate_aum(self, aum_list):
        self.dynamic_ls = True
        self.smooth_rates = np.zeros(len(self.poison_indicator))
        for i in range(len(self.poison_indicator)):
            if aum_list[i] > 0:
                self.smooth_rates[i] = aum_list[i] / 2

    def set_dynamic_smooth_rate_ls_aum(self, p_rate, n_rate, aum_list):
        self.dynamic_ls = True
        self.smooth_rates = np.zeros(len(self.poison_indicator))
        max_aum = max(aum_list)
        min_aum = min(aum_list)
        aum_range = max_aum - min_aum
        rate_range = p_rate - n_rate
        logging.info(f'max_aum:{max_aum}, aum_range:{aum_range}, rate_range:{rate_range}')
        for i in range(len(self.poison_indicator)):
            self.smooth_rates[i] = n_rate + rate_range*((aum_list[i]-min_aum)/aum_range)

    def set_dynamic_smooth_rate(self, p_rate, n_rate):
        self.dynamic_ls = True
        self.smooth_rates = np.zeros(len(self.poison_indicator))
        self.p_rate = p_rate
        self.n_rate = n_rate
        for i in range(len(self.poison_indicator)):
            if self.poison_indicator[i] > 0:
                self.smooth_rates[i] = n_rate
            else:
                self.smooth_rates[i] = p_rate

    def set_dynamic_smooth_rate_byIndex(self, p_rate, n_rate, p_rate_list):
        assert len(self.poison_indicator) == len(p_rate_list)
        self.dynamic_ls = True
        self.smooth_rates = np.zeros(len(p_rate_list))
        self.p_rate = p_rate
        self.n_rate = n_rate
        for i in range(len(p_rate_list)):
            if p_rate_list[i] > 0:
                self.smooth_rates[i] = n_rate
            else:
                self.smooth_rates[i] = p_rate
    ...
    def __getitem__(self, item):
        ...
        if self.add_details_in_preprocess and self.dynamic_ls:
            return img, label, self.original_index[item], self.poison_indicator[item], \
                   self.original_targets[item], self.smooth_rates[item]
    ...
    def __len__(self):
        if self.add_details_in_preprocess:
            all_length = (len(self.data), len(self.targets), len(self.original_index), len(self.poison_indicator),
    ...
    def subset(self,
        ...
        if inplace:
            ...
            if self.add_details_in_preprocess:
                ...
                if self.dynamic_ls:
                    self.set_dynamic_smooth_rate(self.p_rate, self.n_rate)
