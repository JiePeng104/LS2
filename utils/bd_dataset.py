...
class prepro_cls_DatasetBD(torch.utils.data.dataset.Dataset):
    ...
    def set_dynamic_smooth_rate_to2Part_aum(self, aum_list):
        self.dynamic_ls = True
        tmp = np.array(aum_list)
        sorted_idx = sorted(range(len(tmp)), key=lambda k: tmp[k])
        sorted_idx = np.array(sorted_idx)
        num = len(aum_list) // 2
        split = np.split(sorted_idx, [num])
        self.smooth_rates = np.zeros(len(self.poison_indicator))
        soft_list = [0.1, 0.7]
        i = 0
        for a in split:
            for idx in a:
                self.smooth_rates[idx] = soft_list[i]
            i += 1

    def set_dynamic_smooth_rate_to3Part_aum(self, aum_list):
        self.dynamic_ls = True
        tmp = np.array(aum_list)
        sorted_idx = sorted(range(len(tmp)), key=lambda k: tmp[k])
        sorted_idx = np.array(sorted_idx)
        num = len(aum_list) // 3
        split = np.split(sorted_idx, [num, 2*num])
        self.smooth_rates = np.zeros(len(self.poison_indicator))
        soft_list = [0.1, 0.4, 0.7]
        i = 0
        for a in split:
            for idx in a:
                self.smooth_rates[idx] = soft_list[i]
