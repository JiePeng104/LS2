        ...
        if clean_label == False:
            ...
            if p_num is not None or round(pratio * len(targets)):
                if p_num is not None:
                    ...
                    non_zero_array = np.random.choice(np.arange(len(targets)), round(pratio * len(targets)),
                                                      replace=False)
                    pidx[list(non_zero_array)] = 1
        else:
            if p_num is not None or round(pratio * len(targets)):
                if p_num is not None:
                    ...
                    non_zero_array = np.random.choice(np.where(targets == tlabel)[0], round(pratio * len(targets)),
                                                      replace=False)
                    pidx[list(non_zero_array)] = 1
...
def generate_pidx_from_label_transform(
    ...
    if isinstance(label_transform, AllToOne_attack):
        ...
        if p_num is not None:
            ...
            select_position = np.random.choice(len(original_labels), size=round(len(original_labels) * pratio),
                                               replace=False)
        else:
