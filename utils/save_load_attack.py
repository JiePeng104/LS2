...
def save_attack_result(
    ...
    if bd_train is not None:
        bd_train_x, bd_train_y, bd_train_original_index, bd_train_poison_indicator, bd_train_original_targets = add_resize_and_subset_for_prepro_cls_DatasetBD(
            bd_train, img_size, only_bd=True)
    else:
        logging.info('bd_train is set to be None in saving process!')

    bd_test_x, bd_test_y, bd_test_original_index, bd_test_poison_indicator, bd_test_original_targets = add_resize_and_subset_for_prepro_cls_DatasetBD(
        bd_test, img_size, only_bd=True)
    ...
    s_name = model_name.split('-')
    if len(s_name) > 1 and s_name[1] == 'TS':
        result_name = 'attack_result-TS.pt'
    else:
        result_name = 'attack_result.pt'
    torch.save(
        save_dict,
...
def save_attack_result_curve_withoutIMG(
        attack_args,
        model_name: str,
        num_classes: int,
        model: dict,  # the state_dict
    ...
    result_name = 'curve_result_withINFO.pt'
    torch.save(
        save_dict,
        f'{save_path}/{result_name}',
    )
...
def load_attack_result(
    ...
    debug_file_path_for_load = debug_info_folder_path + '/' + time.strftime("%Y_%m_%d_%H_%M_%S",
                                                                            time.localtime()) + '_load_debug.log'
    ...
    if all(key in load_file for key in ['model_name',
        ...
        clean_train_x, clean_train_y, _, _, _ = add_resize_and_subset_for_prepro_cls_DatasetBD(clean_train_ds,
                                                                                               clean_setting.img_size)
        ...
        clean_test_x, clean_test_y, _, _, _ = add_resize_and_subset_for_prepro_cls_DatasetBD(clean_test_ds,
                                                                                             clean_setting.img_size)

        if (load_file['bd_train']['x'] is not None) and (load_file['bd_train']['y'] is not None) and (
            ...
            if load_file['bd_train']['y'].__len__() != load_file['bd_train']['original_index'].__len__() and load_file[
                'bd_train'].get('poison_indicator') is not None:
        ...
        print(
            f"location : {save_path}, content summary :{pformat(summary_for_load_without_model)}")  # ignore model info for print only
...
def load_attack_result_curve(
    ...
    if all(key in load_file for key in ['model_name',
        ...
        load_dict = {
            ...
            'attack_args': load_file['attack_args'],
        }
